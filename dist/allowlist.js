...
export async function loadAllowlist(configPath) {
...
export async function saveAllowlist(config, configPath) {
    const filePath = configPath || defaultAllowlistPath;
    await fs.mkdir(path.dirname(filePath), { recursive: true });
    await fs.writeFile(filePath, JSON.stringify(config, null, 2), { mode: 0o600 });
    return filePath;
}
export async function allowlistExists(configPath) {
    const filePath = configPath || defaultAllowlistPath;
    try {
...
export function evaluateAllowlist(config, toAddress, amountMicro, memo) {
